#!/usr/bin/env python3
"""
Sync essential data to staging (without large image fields)
"""
...
def main():
    ...
    print("🔄 Essential Data Sync to Staging")
    print("=" * 40)
    ...
    try:
        ...
        with prod_engine.connect() as prod_conn, staging_engine.connect() as staging_conn:
            ...
            orgs = prod_conn.execute(text("SELECT id, name FROM organization")).fetchall()
            for org in orgs:
                staging_conn.execute(text("INSERT INTO organization (id, name) VALUES (:id, :name)"), 
                                   {"id": org[0], "name": org[1]})
            staging_conn.commit()
            ...
            # Sync ranks
            print("📤 Syncing ranks...")
            ranks = prod_conn.execute(text("SELECT id, name FROM rank")).fetchall()
            for rank in ranks:
                staging_conn.execute(text("INSERT INTO rank (id, name) VALUES (:id, :name)"), 
                                   {"id": rank[0], "name": rank[1]})
            staging_conn.commit()
            ...
            print(f"\n✅ Essential data sync completed successfully!")
